...
- the MQTT topic prefix rename to `play-presence`: legacy retained topics are cleared on connect only when off the legacy prefix, a live `rg40xxv` deployment is never disturbed, and the installer migrates only the exact legacy default `topic_prefix` and `client_id` while preserving custom values, each field independently
