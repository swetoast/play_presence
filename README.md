...
Play Presence lets Home Assistant know which game is running on an Anbernic handheld. It detects games started through RetroArch or a native emulator, cleans up ROM filenames, finds matching local box art, and publishes the result over MQTT.
...
On the first installation, Play Presence asks for the MQTT password in the terminal and hides the input while you type. The installer downloads the current source into `/tmp`, validates the project layout, installs and starts the service, removes temporary files, and prints the installed version and service status.
...
MQTT discovery creates these entities under one device:

- **Current game**: the resolved game title
- **Playing**: whether a game is running
- **System**: the emulated platform, when enabled
- **Current game artwork**: the matching local box art

The retained state message stays focused on the game:
...
Play Presence deliberately keeps software version, detection method, filesystem paths, battery information, performance data, and other diagnostics out of the normal game state.
...
1. The matching `<name>` in the system `gamelist.xml`
2. An optional manual title override
3. A cleaned version of the ROM filename

Filename normalization removes only patterns that are known to be technical metadata rather than part of the title. This includes stacked ROM and archive extensions, recognized region and language tags, revision markers, beta or prototype labels, and known dump tags. Underscores and repeated whitespace are normalized, and a trailing article such as `, The`, `, A`, or `, An` is moved to the front.

Normalization is intentionally conservative. Unknown parentheses, punctuation, hack names, special-edition labels, collection names, and separators such as `+`, `&`, hyphens, and colons are preserved. When Play Presence cannot confidently identify text as metadata, the text remains part of the displayed title.

## Artwork

Play Presence first checks the `<image>` value for the exact matching game in `gamelist.xml`. If that image is unavailable, Play Presence checks the system's mirrored `images` directory, including nested ROM folders.
...
Before publishing an image, Play Presence verifies that the file is regular, is not a symlink, remains inside the active system directory, has a supported extension and matching signature, is not empty, and does not exceed the configured limit. The default limit is 2 MiB.
...
Check the game's `<image>` value in `gamelist.xml` or the corresponding system `images` folder. The image must match the expected ROM-relative artwork name and pass the file checks described above.

### A fallback title still contains extra text

Play Presence removes only metadata patterns it recognizes. Leaving unfamiliar text in place is safer than removing part of a real title.
